...
async function mimeHtmlAsync(message, options = {}) {
    ...
    try {
        // Pre-process HTML to remove known problematic CSS that causes juice to hang
        // This is the same filtering as the sync version to avoid unnecessary timeouts
        const tempDoc = new JSDOM(html).window.document;
        ...
        // Note: logErrors option can be used to control error visibility
...
// Close worker pool when process exits
if (globalWorkerPool) {
    process.on('exit', () => {
        if (globalWorkerPool) {
            globalWorkerPool.close().catch(() => {});
        }
    });
}
