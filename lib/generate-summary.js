...
async function generateSummary(message, apiToken) {
    ...
    if (message.html && (!text || message.html.length > text.length * 10)) {
    ...
    let header = [];
    if (message.subject) {
        header.push(`Subject: ${message.subject}`);
    }
    if (message.from) {
        let from = [].concat(message.from || []).shift();
        header.push(`From: ${from.name || from.address}`);
    }
