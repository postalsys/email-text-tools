...
async function concurrencyBenchmark() {
    ...
    for (const concurrency of concurrencyLevels) {
        ...
        for (let i = 0; i < totalRequests; i++) {
            poolPromises.push(
                mimeHtml.async({ html: realWorldHtml }, {
                    useWorkerPool: true,
                    timeout: 5000,
                    minWorkers: 2,
                    maxWorkers: 4,
            ...
            if (poolPromises.length >= concurrency) {
                await Promise.race(poolPromises).then(() => {
                    const index = poolPromises.findIndex(p => 
    ...
    const bestAsync = results
        .filter(r => r.method === 'Async (pool)')
        .sort((a, b) => parseFloat(a.totalTime) - parseFloat(b.totalTime))[0];
