...
- Your analysis should contain (but is not limited to) the following:
  ...
  - Does the sender email look like it might be spoofed
...
- Respond with a JSON formatted structure with numeric risk score as "risk" property, and the assessment as the "assessment" property. Do not write any other explanations.

Facts:
- An email consists of message headers, attachments list, and text content
- An email might contain an "authentication-results" result header that indicate if the message passed SPF, DKIM, ARC and DMARC checks
...
${JSON.stringify(content)}`;
        ...
        if (tokens.text.length <= MAX_ALLOWED_TOKENS) {
            break;
        }
